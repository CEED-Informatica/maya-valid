...
from validation import STUDIES_VAL
...
class StudiesValidation(models.Model):
  """
  Define la entrega de convalidaciones de estudios por parte del alumnado
  """
  _name = 'maya_valid.studies_validation'
  _description = 'Solicitud convalidación por estudios'
  _inherit = ['maya_valid.validation']
  
  validation_subjects_ids = fields.One2many('maya_valid.studies_validation_subject', 'validation_id', 
     string = 'Módulos que se solicita convalidar')

  validation_subjects_not_for_correction_ids = fields.One2many('maya_valid.studies_validation_subject', 
  'validation_id', 
    string = 'Módulos que se solicita convalidar', 
    domain = [('state', '!=', '1')],
    compute = '_compute_validation_subjects_not', readonly = False)

  validation_subjects_for_correction_ids = fields.One2many('maya_valid.studies_validation_subject', 'validation_id', 
  ...
  def download_validation_action(self):
    ...
    """  
    self.download_validation_action(self, STUDIES_VAL)
